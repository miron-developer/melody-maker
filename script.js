...
function onMouseDown() {
  MOUSE.isDown = true;
  const prop = getNoteAtLocation(MOUSE);
...
function getNoteAtLocation(location) {
  const index = Math.round(location.y / SPACING);
  return {
    x: MARGIN_RIGHT,
    y: index * SPACING,
    freq: FREQ[index],
...
class Clef {
  ...
  draw(ctx) {
    ...
    const newHeight = CANVAS.height * 0.65;
...
class MovingNote {
  ...
  draw(ctx) {
    const color = this.isAuto ? PURPLE : BLUE;
  ...
  update() {
    const timeDiff = new Date().getTime() - this.born;
    const noteType = Math.floor(timeDiff / HOLD_DURATION);
    switch (noteType) {
      case 0:
        this.duration = 0.5;
        break;
